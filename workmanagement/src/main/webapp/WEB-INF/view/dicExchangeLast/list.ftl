	...
	<body>
		...
		<#if reportIndexErrorList?? !>
			...
			<div class="queryInputBox" style="margin-bottom:10px">
				<div class="marginT10">
					<input class="sureBtn sureBtnEx " 
					onclick="setLayer('新增汇率','${request.getContextPath()}/admin/dicExchangeLast/edit.jhtml');$(this).blur();"
					type="button" value="新增汇率"  style="margin-left:30px"/>
					<form  id="upload" method="post"  enctype="multipart/form-data" action="${request.getContextPath()}/admin/dicExchangeLast/upLoad.jhtml" style="display: inline-block;*zoom=1;*display:inline" class="marginL20">
						<input class="inputSty" id="file" type="file" name="file" value="上传"  />
						<input type="submit" value="批量导入" class="sureBtn" style="margin-left:0px"/>
					</form>
					<input type="button" value="下载模板" class="sureBtn" onclick="download()"/>
					
					<input type="button" value="批量导出" class="sureBtn" onclick="setLayer('批量导出','${request.getContextPath()}/admin/dicExchangeLast/batchexport.jhtml');$(this).blur();"/>
				</div>
						...
						<#list dicExchangeLastList as item>
							<tr>
								...
								<td>
									...
									onclick="setLayer('查看历史','${request.getContextPath()}/admin/dicExchangeLast/dicExchangeHistoryList.jhtml?dicExchangeId=${item.dicExchangeId}&dicExchangeName=${item.dicExchangeName}&qb=${qb}&dicAreaId=${item.dicAreaId}')">查看历史</a> 
