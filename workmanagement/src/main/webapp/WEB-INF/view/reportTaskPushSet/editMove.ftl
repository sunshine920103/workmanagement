    ...
    <script type="text/javascript">
		...
		$(function(){
			...
			$("#rightMove").click(function(){//右移
				if(html!="" && html.substring(0,1)!="L"){
					$("#leftTabel").append(html.substring(1));
					$("#rightTabel tr:eq("+indexNum+")").remove();
					html="";
				}
			})
			$("#leftMove").click(function(){//左移
				if(html!="" && html.substring(0,1)!="R"){
					$("#rightTabel").append(html.substring(1));
		...
		//选中机构
			function orgNameClick(obj,orgid){
				...
				if(tableId=="leftTabel"){
					html="L<tr><input type='hidden' name='orgIds' value="+orgid+">"
					html+=obj.parents("tr").html()+"</tr>";
				}else if(tableId=="rightTabel"){
					html="R<tr><input type='hidden' name='orgIds' value="+orgid+">"
					html+=obj.parents("tr").html()+"</tr>";
				}
				//判断是否有展开图标 有则表示有下级
//				if(obj.parent().find("img").length>0){
//					
//				}
			}
			/********************************************************
    ...
    <table cellpadding="0" cellspacing="0">
        ...
        <tr>
            <td colspan="2">
                ...
                <div class="choiceBtn">
                    <input class="sureBtn sureBtnEx" type="button" id="rightMove" value="右移>>" style="width: 60px;"> <br>
                    <input class="sureBtn sureBtnEx" type="button" id="leftMove" value="<<左移" style="width: 60px;">
                </div>
