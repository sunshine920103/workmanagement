...
<script type="text/javascript">
	...
	function objManage(){
		...
		var sysOperateId=$("#sysOperateId").val();
		check_val=[];
		obj=$(".ckName");
		for(k in obj){
			if(obj[k].checked){
	    ...
	    params.reportOrgId = reportOrgId;
	    params.sysOperateId = sysOperateId;
	    $.ajax({  
	        async:false,  
	        type: "POST",  
	        url: "${request.getContextPath()}/admin/orgObj/operate.jhtml",//注意路径  
	        data:params,  
	        dataType:"json",  
	        success:function(data){  
	            if(data.msg=="msg"){
	            	...
	            	$("#sysOperateList").val(data.sysOperateList);
	            	$(":checkbox:checked").each(function(){
						if($(this).siblings(".inputSty").attr("disabled")=="disabled"){
							...
							var str = "<tr><td class='noBorderL firstTD' >报送机构说明</td><td class='secondTD'><input name='operateOrgdesc' value='' class='inputSty'  /></td></tr><tr><td class='noBorderL firstTD' >信息主体说明</td><td class='secondTD'><input name='operateInformation' value='' class='inputSty'  /></td></tr>";
...
//    	parent.window.location.href="${request.getContextPath()}/admin/orgObj/list.jhtml?msgs="+msg;
...
<div id="poplayer">
	...
	<div div class="showListBox noBorder" >
	<form enctype="multipart/form-data" method="post" action="${request.contextPath}/admin/orgObj/save.jhtml">
		<table width="70%" id="indexLastValueTable" cellspacing="0" cellpadding="0">
			<#if myIndexItemTbList?? !>
				...
				<input type="hidden" id="sysOperateList" name="sysOperateList" value=""/>
				<input type="hidden" id="sysOperateId" name="sysOperateId" value="${sysOperateId}"/>
				<input type="hidden" id="shouAndrevise" name="shouAndrevise" value="1"/>
				<input type="hidden" id="reportOrgId" name="reportOrgId" value="${mapSon.SYS_ORG_ID}"/>
				<tr>
					<td  width='100' class='noBorderL firstTD'>统一社会信用代码</td>
					...
					<tr>
						...
						<td  class="secondTD">
							<#if item.indexItemType==1>
								...
								<span class="fontSize12">服务中心说明：</span>
								<input class="message" type="text" value="" disabled="disabled"/>
							<#elseif item.indexItemType==3>
								...
								<#elseif item.dicType==5>
									<input class="inputSty dq" type="text" onclick="openPop(5)" name="indexItemValue" value="${myValueList[item_index]}" />
									<input class="inputSty" type="hidden" name="indexItemCode" value="${item.indexItemCode}" />
									<input type="checkbox" name="ckName" class="ckName" value="${item.indexItemId}"
									<span class="fontSize12"></span>
									<span class="fontSize12">服务中心说明：</span>
								...
								<#elseif item.dicType==3>
									<input class="inputSty zf" type="text" onclick="openPop(3)" name="indexItemValue" value="${myValueList[item_index]}" />
									<input class="inputSty" type="hidden" name="indexItemCode" value="${item.indexItemCode}" />
									<input type="checkbox" name="ckName" class="ckName" value="${item.indexItemId}"
									<span class="fontSize12"></span>
									<span class="fontSize12">服务中心说明：</span>
								...
								<#elseif item.dicType==2>
									<input class="inputSty jg" type="text" onclick="openPop(2)" name="indexItemValue" value="${myValueList[item_index]}" />
									<input class="inputSty" type="hidden" name="indexItemCode" value="${item.indexItemCode}" />
									<input type="checkbox" name="ckName" class="ckName" value="${item.indexItemId}"
									<span class="fontSize12"></span>
									<span class="fontSize12">服务中心说明：</span>
								...
								<#elseif item.dicType==1>
									<input class="inputSty hy" type="text" onclick="openPop(1)" name="indexItemValue" value="${myValueList[item_index]}" />
									<input class="inputSty" type="hidden" name="indexItemCode" value="${item.indexItemCode}" />
									<input type="checkbox" name="ckName" class="ckName" value="${item.indexItemId}"
									<span class="fontSize12"></span>
									<span class="fontSize12">服务中心说明：</span>
		...
		<div class="showBtnBox">
			<input type="button" value="异议处理" style="background: #d4281b;" class="sureBtn sureBtnEx marginR40"  onclick="objManage()">
			<input type="button" value="终 止" class="sureBtn sureBtnEx marginR40" style="background: #d4281b;" onclick="objSave()">
