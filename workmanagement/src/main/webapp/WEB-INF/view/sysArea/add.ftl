...
<html>
		...
		<div class="showListBox">
			...
			<table cellpadding="0" cellspacing="0">
					...
					<tr>
						...
						<#if poptype == "add" >
							<td width="500" class="secondTD">
								<input id="pname" name="pname" type="hidden" value="${a.sysAreaName}"/>
								<input id="sysAreaUpid" name="sysAreaUpid" type="hidden" value="${a.sysAreaId}"/>
								<a id="openPop" href="javascript:" style="color:rgb(56,165,226);padding:5px;font-size:12px" onclick="openPop()">${a.sysAreaName}</a>
					...
					<tr>
						...
						<#else>
						<td class="secondTD" style="position: relative;">
							<input id="sysAreaName" name="sysAreaName" class="inputSty required inputOtherCondition allChineseVal" value="${a.sysAreaName}" maxlength="50" onblur="onblurVal(this,1,50)" title="必填，不能超过50个字"/>
						</td>
						</#if>
					</tr>
					<tr> 
						...
						<#if poptype == "add">
							...
							<input id="sysAreaCode" name="sysAreaCode" class="inputSty required" maxlength="6" value="${a.sysAreaCode?substring(0,2)}" onblur="onblurVal(this,246,6)" />
							</td>
							<#elseif a.sysAreaType == '1'>
							<td class="secondTD" id="Areacode"> 
							<input id="sysAreaCode" name="sysAreaCode" class="inputSty required" maxlength="6" value="${a.sysAreaCode?substring(0,4)}" onblur="onblurVal(this,246,6)" />
							</td>
							<#else>                                                                                                    
							<td class="secondTD" id="Areacode"> 
							<input id="sysAreaCode" name="sysAreaCode" class="inputSty required" maxlength="9" value="${a.sysAreaCode}" onblur="onblurVal(this,249,9)" />
							</td>
							</#if>
						<#else>
							<td class="secondTD" id="Areacode"> 
	...
	<script>	
			...
			$("#submitBtn").click(function(){
				...
				if($("#areaType").val()==3){
					if(administrativeCode9(sysAreaCode)==0) {
						layer.alert("行政代码输入不合法",{icon:2,shade:0.3,shouldClose:true});
		                $("#sysAreaCode").focus();
		                return false;
					};
				}else{
					if(administrativeCode6(sysAreaCode)==0) {
						layer.alert("行政代码输入不合法",{icon:2,shade:0.3,shouldClose:true});
		                $("#sysAreaCode").focus();
		                return false;
					};
